...
public:
	Thread() : tid(0), running(0), detached(0), nextSlot(0){}
	...
	virtual ~Thread() {
		...
		for (Node* curNode : node_pool) {
			delete curNode;
		}
	}

	int start(int cpuid, bool xHandler) {
		pthread_attr_t attr;
...
//#else
	...
	Node* returnNextNode(int r_id, int p_id, JOB_TYPE j_type) {
		if (nextSlot < 50) {
			//printf("new node...\n");
