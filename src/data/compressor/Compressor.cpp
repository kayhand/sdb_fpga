...
bool Compressor::parseColumn(){
	...
	if(!file){
		std::cout << "File not found in path " << raw_path << std::endl;
		return false;
	}
	...
	std::cout << "Parsed column in path %s" << raw_path << std::endl;
...
void Compressor::writeCompressedValues(){
	...
	std::cout << "Compressed values written into " << compressed_path << std::endl;
	in_file.close();
	out_file.close();
}
...
void Compressor::writeMetaData(){
	...
	std::cout << "Metadata written into " << meta_path << std::endl;
}
